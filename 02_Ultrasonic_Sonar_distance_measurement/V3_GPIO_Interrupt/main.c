...
void configure_timer() {
    ...
    // Changed to Periodic Mode (0x02) + Count Up (0x10)
    TIMER1_TBMR_R = 0x12;           
...
void configure_pins() {
    ...
    // Disable Alternate Function (it is a standard GPIO now)
    GPIO_PORTD_AHB_AFSEL_R &= ~(1 << 3);         
    
    // Configure GPIO Interrupts for PD3
    GPIO_PORTD_AHB_IM_R &= ~(1 << 3);            // Mask interrupt during setup
    GPIO_PORTD_AHB_IS_R &= ~(1 << 3);            // Edge-sensitive
    GPIO_PORTD_AHB_IBE_R |= (1 << 3);            // Trigger on BOTH edges
    GPIO_PORTD_AHB_ICR_R = (1 << 3);             // Clear any prior flags
    GPIO_PORTD_AHB_IM_R |= (1 << 3);             // Unmask (Enable) interrupt

    NVIC_EN0_R |= (1 << 3);                      // Enable NVIC for IRQ 3 (GPIO Port D)
...
void myInterrupt(void) {
    GPIO_PORTD_AHB_ICR_R = (1 << 3);   // Acknowledge and clear the interrupt flag for PD3

    if (capture_state == 1) {
        start_time = TIMER1_TBR_R;     // Look at the running timer to get the time
        capture_state = 2; 
    } 
    else if (capture_state == 2) {
        end_time = TIMER1_TBR_R;       // Look at the running timer again
